...
from mt_economic_common.currency.managers.fx_rate_update_factory import (
    FxRateUpdateFactory,
)
...
class FxRateUpdateFactoryTest(TestCase):
    def test_get_fx_rate_upload_strategy_unkown(self):
        with self.assertRaises(ValueError) as error:
            ...
            self.assertEqual(
                str(error.exception), "Unknown fx rate upload strategy for Unknown"
            )

    def test_get_fx_rate_upload_strategy_yahoo(self):
        self.assertTrue(
            isinstance(
                FxRateUpdateFactory.get_fx_rate_update_strategy("Yahoo"),
                YahooFxRateUpdateStrategy,
            )
        )
